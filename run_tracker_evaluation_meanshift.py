...
def main():
    ...
    if evaluation.video == 'all':
        ...
        for i in range(1,len(images_arr)):
            ...
            frame_i = images_arr[i]
            roi = frame_i[r:r+h, c:c+w]
            hsv_roi =  cv2.cvtColor(frame_i, cv2.COLOR_BGR2HSV)
            ...
            bboxes_final[i,:] = x, y, w, h
            
            fig = plt.figure(1)
            ax = fig.add_subplot(111)
            r = patches.Rectangle((x,y), w, h, linewidth=2, edgecolor='r', fill=False)
            ax.imshow(np.uint8(frame_i))
            ax.add_patch(r)
            plt.ion()
            plt.show()
            plt.pause(0.001)
